...
- **R3.3** Focus override / lock takeover (`confirmFocusOverride` 2090): when held by other LIVE session, auto-proceed when `shouldAutoConfirmProposal` is true — i.e. in non-TUI (regardless of env) OR when `PI_GOAL_AUTO_CONFIRM=1` is set explicitly → `releaseLock` + notify warning. This reuses the existing proposal-auto-confirm semantics so non-TUI launches get full goal functionality without a prompt. TUI without opt-in still prompts via `ctx.ui.confirm`.
